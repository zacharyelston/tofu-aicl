...
class NagaProvider(provider_pb2_grpc.ProviderServicer):
    ...
    def Configure(self, request, context):
        ...
        if not self.api_key:
            diag = self._create_diagnostic(provider_pb2.Diagnostic.ERROR, "NAGA_API_KEY environment variable not set")
    ...
    def ApplyResourceChange(self, request, context):
        ...
        if request.type_name in ["embedding", "naga_embedding", "openrouter_embeddings"]:
            return self._generate_embeddings(resource_id, config, request.type_name)

        # Handle chat/query
        elif request.type_name in ["chat", "naga_chat", "openrouter_query"]:
